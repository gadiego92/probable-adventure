...
void messageCb(const geometry_msgs::Vector3 &vector)
{
  if (speed != vector.x)
  {
    speed = vector.x;
  ...
  {
    angle = vector.y;
    SerialServoMove(serial_servo, ID3, angle, TURN_TIME);
  }
}

ros::Subscriber<geometry_msgs::Vector3> sub("rover_motion", &messageCb);
